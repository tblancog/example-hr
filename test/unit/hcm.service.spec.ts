...
describe('HcmService', () => {
  ...
  describe('getBalance()', () => {
    ...
    it('throws InvalidDimensionException when HCM returns 404', async () => {
      ...
      jest.spyOn(hcmService, 'getBalance').mockRejectedValueOnce(new InvalidDimensionException('emp-bad', 'loc-bad'));

      await expect(hcmService.getBalance('emp-bad', 'loc-bad')).rejects.toBeInstanceOf(InvalidDimensionException);
    });
    ...
    it('throws HcmUnavailableException when HTTP request times out', async () => {
      ...
      jest.spyOn(hcmService, 'getBalance').mockRejectedValueOnce(new HcmUnavailableException('timeout'));

      await expect(hcmService.getBalance('emp-001', 'loc-nyc')).rejects.toBeInstanceOf(HcmUnavailableException);
    });

    it('throws HcmUnavailableException when HCM returns 500', async () => {
      const { HcmUnavailableException } = await import('src/common/exceptions/hcm-unavailable.exception');

      jest.spyOn(hcmService, 'getBalance').mockRejectedValueOnce(new HcmUnavailableException('internal_server_error'));
  ...
  describe('setBalance()', () => {
    it('returns updated balance on success', async () => {
      ...
      const result = await hcmService.setBalance('emp-001', 'loc-nyc', { available: 7, used: 5, total: 12 });
    ...
    it('throws InsufficientBalanceException when HCM returns 400 with insufficient_balance', async () => {
      ...
      jest.spyOn(hcmService, 'setBalance').mockRejectedValueOnce(new InsufficientBalanceException(0, 3));

      await expect(
        hcmService.setBalance('emp-001', 'loc-nyc', { available: -3, used: 15, total: 12 }),
    ...
    it('throws InvalidDimensionException when HCM returns 422', async () => {
      ...
      jest.spyOn(hcmService, 'setBalance').mockRejectedValueOnce(new InvalidDimensionException('emp-001', 'loc-bad'));

      await expect(
        hcmService.setBalance('emp-001', 'loc-bad', { available: 5, used: 0, total: 5 }),
    ...
    it('throws HcmUnavailableException when HCM returns 500', async () => {
      ...
      jest.spyOn(hcmService, 'setBalance').mockRejectedValueOnce(new HcmUnavailableException('internal_server_error'));

      await expect(
        hcmService.setBalance('emp-001', 'loc-nyc', { available: 5, used: 0, total: 5 }),
